...
- CRITICAL — filtering provinces.name_vi / districts.name_vi: stored values include prefixes ("Thành phố Hà Nội", "Tỉnh Bắc Ninh", "Quận Ba Đình", "Phường Hoàn Kiếm", "Xã Tân Triều") while users say short forms ("Hà Nội", "Bắc Ninh", "Ba Đình", "Hoàn Kiếm"). ALWAYS use `name_vi ILIKE '%keyword%'`. NEVER use `=`, `IN (...)`, or any exact match. For multi-place comparisons, build `(name_vi ILIKE '%X%' OR name_vi ILIKE '%Y%' OR ...)`. Treat "Sài Gòn" as "Hồ Chí Minh".
