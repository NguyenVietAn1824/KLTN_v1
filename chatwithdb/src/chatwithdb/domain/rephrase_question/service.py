...
from chatwithdb.shared.models.state import RephraseServiceState
from chatwithdb.shared.settings import RephraseQuestionSettings
...
class RephraseService(BaseService):
    ...
    def preprocess_memory(
        ...
        """
        try:
            recent_turns_txt: str = ''
            for turn in recent_turns[::-1]:
                turn = turn['qa_list']
                question_content = self.sanitize(turn[0]['question'])
                answer_content = self.sanitize(turn[1]['answer'])
                recent_turns_txt += f'{MessageRole.USER.value}: {question_content}\n{MessageRole.ASSISTANT.value}: {answer_content}\n'
        ...
        try:
            rephrase_results = await self.process(
                inputs=RephraseServiceInput(
                    ...
                    conversation_history=state.get('history_retrieval_state', {}).get('conversation_memories', []),
                    summary=state.get('history_retrieval_state', {}).get('conversation_summary', ''),
                ),
            )
